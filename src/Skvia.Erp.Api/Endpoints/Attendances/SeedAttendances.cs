        ...
        for (int i = 0; i <= 30; i++)
        {
            ...
            foreach (var employee in employees)
            {
                ...
                if (random.Next(100) < 80)
                {
                    ...
                    var checkInProperty = typeof(Skvia.Erp.Domain.Attendances.Attendance).GetProperty(nameof(attendance.CheckIn));
                    checkInProperty?.SetValue(attendance, new DateTimeOffset(checkInUtc, TimeSpan.Zero));
                    
                    var dateProperty = typeof(Skvia.Erp.Domain.Attendances.Attendance).GetProperty(nameof(attendance.Date));
                    ...
                    if (random.Next(100) < 90)
                    {
                        ...
                        var checkOutProperty = typeof(Skvia.Erp.Domain.Attendances.Attendance).GetProperty(nameof(attendance.CheckOut));
